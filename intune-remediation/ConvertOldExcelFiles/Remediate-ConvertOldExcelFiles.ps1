<#
.SYNOPSIS
    Intune Remediation - Remediation: Convert old Excel (.xls) to .xlsx in OneDrive and move originals to converted_old.

.DESCRIPTION
    Runs when detection found legacy .xls files. Converts each to .xlsx using Excel COM, then moves
    the original into a local "converted_old" folder. Runs in SYSTEM context; processes all user
    OneDrive folders. Requires Excel installed.
...
try {
    ...
    foreach ($oneDrivePath in $oneDrivePaths) {
        $excelFiles = Get-ChildItem -Path $oneDrivePath -Recurse -Include "*.xls" -File -ErrorAction SilentlyContinue |
    ...
    Write-Output "Remediation completed. Converted $totalConverted .xls file(s)."
    exit 0
} catch {
