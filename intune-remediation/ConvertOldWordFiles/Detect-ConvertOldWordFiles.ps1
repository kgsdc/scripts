<#
.SYNOPSIS
    Intune Remediation - Detection: Old Word (.doc) files in OneDrive that need conversion to .docx.

.DESCRIPTION
    Returns exit 0 if no legacy .doc files are found (compliant). Exit 1 if any .doc files exist
...
try {
    ...
    foreach ($path in $oneDrivePaths) {
        $files = Get-ChildItem -Path $path -Recurse -Include "*.doc" -File -ErrorAction SilentlyContinue |
            Where-Object { $_.FullName -notmatch [regex]::Escape("\$ConvertedFolderName\") }
        $legacyCount += ($files | Measure-Object).Count
    }
    if ($legacyCount -gt 0) {
        Write-Output "Detected $legacyCount legacy .doc file(s) in OneDrive requiring conversion."
        exit 1
    }
    Write-Output "No legacy .doc files found. Compliant."
    exit 0
} catch {
